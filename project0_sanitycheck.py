# project0_sanitycheck.py
#
# ICS 32 Winter 2022
# Project #0: Square One
#
# This is a sanity checker for your Project #0 solution, which checks whether
...
def run_test() -> None:
    ...
    try:
        ...
        print_labeled_output(
            'PASSED',
            'Your "project0.py" passed the sanity checker.  Note that there are',
...
def start_process() -> TextProcess:
    filenames_in_dir = [p.name for p in list(
        pathlib.Path.cwd().iterdir()) if p.is_file()]

    if not 'project0.py' in filenames_in_dir:
        print_labeled_output(
            'ERROR',
            'Cannot find file "project0.py" in this directory.',
            'Make sure that the sanity checker is in the same directory as the',
            '"project0.py" that comprises your Project #0 solution.  Also, be',
            'sure that you\'ve named your "project0.py" file correctly, noting',
        ...
        return TextProcess(
            [sys.executable, str(pathlib.Path.cwd() / 'project0.py')],
